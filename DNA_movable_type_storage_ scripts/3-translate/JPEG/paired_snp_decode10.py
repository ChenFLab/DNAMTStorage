...
while (i < len_snp_sam):
    ...
    if (len(read1_list) == int(max_splice) and len(read2_list) == int(
            max_splice) and duplicate == False and unmatch == False):  ##判断长度是否一致,且不缺失
    ...
    elif ((len(read1_list) <= int(max_splice) or len(read2_list) <= int(
            max_splice)) and duplicate == False and unmatch == False):  ##进行缺失值的初步overlap矫正
        target_read_dict = {"index1": 0, "index2": 0, "index3": 0, "index4": 0, "elem1": 0, "elem2": 0, "elem3": 0,
                            "elem4": 0, "elem5": 0, "check": 0}
